    ...
    try:
        with open(yaml_path, "r") as f:
            ...
            if "categories" in data and isinstance(data["categories"], dict):
                ...
                print(
                    f"Error: 'categories' key not found or not a dictionary in {yaml_path}",
                    file=sys.stderr,
                )
                sys.exit(1)
...
def generate_schema_yaml(db_settings_data, categories):
    ...
    for category_name, settings_list in categories.items():
        ...
        for setting_name in sorted(settings_list):
            ...
            if not setting_data:
                print(
                    f"Warning: Setting '{setting_name}' from YAML not found in database data. Skipping.",
                    file=sys.stderr,
                )
                continue
            ...
            if reset_val is not None:
                try:
                    ...
                    print(
                        f"Warning: Could not convert reset_val '{reset_val}' for setting '{setting_name}'. Error: {e}",
                        file=sys.stderr,
                    )
...
def main():
    ...
    yaml_settings = {
        setting for sublist in yaml_categories.values() for setting in sublist
    }
    print(
        f"Found {len(yaml_settings)} settings in {len(yaml_categories)} categories in the YAML file."
    )
